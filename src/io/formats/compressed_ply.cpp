...
namespace lfs::io {
    ...
    bool is_compressed_ply(const std::filesystem::path& filepath) {
        ...
        for (int i = 0; i < 100 && std::getline(file, line); ++i) {
            if (line.find("element chunk ") == 0)
                has_chunk = true;
            if (line.find("property uint packed_position") != std::string::npos)
                has_packed_position = true;
    ...
    load_compressed_ply(const std::filesystem::path& filepath) {
        ...
        if (sh_coeffs == 9) {
            sh_degree = 1;
            shN_coeffs = 3;
        } else if (sh_coeffs == 24) {
            sh_degree = 2;
            shN_coeffs = 8;
        } else if (sh_coeffs == 45) {
            sh_degree = 3;
            shN_coeffs = 15;
        }
    ...
    write_compressed_ply(const SplatData& splat_data,
        ...
        try {
            ...
            Tensor morton_codes = morton_encode(splat_data.means());
            Tensor sort_indices = morton_sort_indices(morton_codes);
            ...
            tbb::parallel_for(tbb::blocked_range<size_t>(0, chunk_count),
                              [&](const tbb::blocked_range<size_t>& range) {
                                  for (size_t ci = range.begin(); ci < range.end(); ++ci) {
                                      ...
                                      for (int j = 0; j < 3; ++j) {
                                          if (max_pos[j] - min_pos[j] < 1e-10f)
                                              max_pos[j] = min_pos[j] + 1e-10f;
                                          if (max_scale[j] - min_scale[j] < 1e-10f)
                                              max_scale[j] = min_scale[j] + 1e-10f;
                                          if (max_color[j] - min_color[j] < 1e-10f)
                                              max_color[j] = min_color[j] + 1e-10f;
                                      }
