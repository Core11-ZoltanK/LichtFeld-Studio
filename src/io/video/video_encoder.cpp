...
private:
    ...
    void cleanup() {
        if (y_gpu_) { cudaFree(y_gpu_); y_gpu_ = nullptr; }
        if (u_gpu_) { cudaFree(u_gpu_); u_gpu_ = nullptr; }
        if (v_gpu_) { cudaFree(v_gpu_); v_gpu_ = nullptr; }

        if (y_pinned_) { cudaFreeHost(y_pinned_); y_pinned_ = nullptr; }
        if (u_pinned_) { cudaFreeHost(u_pinned_); u_pinned_ = nullptr; }
        if (v_pinned_) { cudaFreeHost(v_pinned_); v_pinned_ = nullptr; }
